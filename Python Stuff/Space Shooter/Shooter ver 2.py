...
pygame.display.set_caption('Spaaaaace')
clock = pygame.time.Clock()

LastTech = pygame.image.load('Bilder/LastTech.png').convert_alpha()
bk = pygame.image.load('Bilder/background.png').convert_alpha()
bk_title = pygame.image.load('Bilder/background_OLD.png').convert_alpha()
option_page = pygame.image.load('Bilder/option.png').convert_alpha()

# Load Bilder
ship = pygame.image.load('Bilder/skip.png').convert_alpha()
ship_borken1 = pygame.image.load('Bilder/skip_skadd1.png').convert_alpha()
ship_borken2 = pygame.image.load('Bilder/skip_skadd2.png').convert_alpha()
ship_Array = [ship, ship_borken1, ship_borken2]

heart = pygame.image.load('Bilder/heart.png').convert_alpha()
shot_anime = pygame.image.load('Bilder/skudd.png').convert_alpha()
shot_anime2 = pygame.image.load('Bilder/skudd2.png').convert_alpha()

LoadingBar = pygame.image.load('Bilder/LoadingBar.png').convert_alpha()
rock_anime = pygame.image.load('Bilder/rock.png').convert_alpha()
rock_anime2 = pygame.transform.scale(rock_anime, (425, 350))

Skill_1_icon = pygame.image.load('Bilder/Skill 1.png').convert_alpha()
Skill_2_icon = pygame.image.load('Bilder/Skill 2.png').convert_alpha()
Skill_3_icon = pygame.image.load('Bilder/Skill 3.png').convert_alpha()
Skill_4_icon = pygame.image.load('Bilder/Skill 4.png').convert_alpha()
Skill_5_icon = pygame.image.load('Bilder/Skill 5.png').convert_alpha()

Skill_1_anime = pygame.image.load('Bilder/Skip_Skill 1.png').convert_alpha()
Skill_2_anime = pygame.image.load('Bilder/Skip_Skill 2.png').convert_alpha()
Skill_3_anime = pygame.image.load('Bilder/Skip_Skill 3.png').convert_alpha()
Skill_4_anime = pygame.image.load('Bilder/Skip_Skill 4.png').convert_alpha()
...
class Rocks(pygame.sprite.Sprite):
    ...
    def move():
        ...
        for i in Rocks.rock_list:
            i.rect.y += i.speed  # Beveger steinene nedover
            if i.rect.y > HEIGHT:
                lvl.rock_kill += 1
...
class Player():
    def __init__(self):
        ...
        self.speed = 3  # Hvor raskt er skipet
        ...
        self.bullet_rate = 2  # Hvor mange skudd du kan skyte i sekundet
        ...
        if self.hurt > 0:
            if Ability_3.skill_active == True:
                ...
                pygame.draw.rect(SCREEN, BLUE,
                                 [self.x, self.y + (self.h / 1.2), percent(self.regen, self.regen_speed * FPS) / 2.5,
                                  8])
                if self.regen == 0:
                    self.regen = self.regen_speed * FPS
                    self.hurt -= 1
        ...
        for i in Boss.boss_list:
            ...
            if i.hp != i.hp_MAX:
                pygame.draw.rect(SCREEN, GREEN,
                                 [i.rect.x + i.w / 2, i.rect.y + i.h / 1.5, (percent(i.hp, i.hp_MAX)), 5])
...
class Level():
    ...
    def draw(self):
        ...
        pygame.draw.rect(SCREEN, BLUE,
                         [WIDTH - 60, HEIGHT - 16, ((percent(self.rock_kill, self.number_rocks_MAX)) / 2), 5])
...
class button():
    ...
    def draw2(self):
        ...
        if self.x + self.w > mouse[0] > self.x and self.y + self.h > mouse[1] > self.y:
            ...
            if self.test_3 == True:
                pygame.draw.rect(SCREEN, self.click_col, (self.x, self.y + 3, self.w, self.h))
            text(self.name + "(" + str(self.price) + "g" + ")", self.text_size, self.text_col,
                 self.x + self.text_size / 2, self.y + 5, self.center)
            self.test_1 = True
            if self.click() == True:
                return (True)
        else:
...
class Ability():
    ...
    def Active_bar(self):
        if self.skill_active == True and self.cooldown_active == False:
            self.duration += 1
            pygame.draw.rect(SCREEN, GREEN,
                             [self.x, self.y + self.h, (50 - (percent(self.duration, self.duration_MAX)) / 2), 5])
    ...
    def Cooldown_bar(self):
        if self.cooldown_active == True:
            ...
            pygame.draw.rect(SCREEN, RED,
                             [self.x, self.y + self.h, (50 - (percent(self.cooldown, self.cooldown_MAX)) / 2), 5])
...
shop_start_level = button(248, 22, 10, HEIGHT - 45 - (25 * 6), GRAY, RED_LIGHT, RED, "Increase start Level", 15, WHITE,
                          False)
...
while lvl.menu:
    ...
    while lvl.shop:
        ...
        if char.ability_1 == True:
            pygame.draw.rect(SCREEN, RED_LIGHT,
                             (shop_ability_1.x, shop_ability_1.y + 3, shop_ability_1.w, shop_ability_1.h))
            SCREEN.blit(Skill_1_icon, (WIDTH - 250, HEIGHT - 150))

        if char.ability_2 == True:
            pygame.draw.rect(SCREEN, RED_LIGHT,
                             (shop_ability_2.x, shop_ability_2.y + 3, shop_ability_2.w, shop_ability_2.h))
            SCREEN.blit(Skill_2_icon, (WIDTH - 175, HEIGHT - 150))

        if char.ability_3 == True:
            pygame.draw.rect(SCREEN, RED_LIGHT,
                             (shop_ability_3.x, shop_ability_3.y + 3, shop_ability_3.w, shop_ability_3.h))
            SCREEN.blit(Skill_3_icon, (WIDTH - 100, HEIGHT - 150))

        if char.ability_4 == True:
            pygame.draw.rect(SCREEN, RED_LIGHT,
                             (shop_ability_4.x, shop_ability_4.y + 3, shop_ability_4.w, shop_ability_4.h))
            SCREEN.blit(Skill_4_icon, (WIDTH - 250 + 37, HEIGHT - 90))

        if char.ability_5 == True:
            pygame.draw.rect(SCREEN, RED_LIGHT,
                             (shop_ability_5.x, shop_ability_5.y + 3, shop_ability_5.w, shop_ability_5.h))
